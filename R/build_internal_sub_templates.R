build_internal_navbar_template_to_html_string <- function(r_options) {
  html_content <- xfun::file_string(get_pathfile_from_res(
...
build_internal_code_menu_template_to_html_string <- function(r_options) {
  js_code_content <- ""
  js_code_menu <- xfun::file_string(get_pathfile_from_res(
    "includes/rmd_to_html",
  ...
  if (r_options$is_folding || r_options$nav$download_code) {
    if (r_options$is_folding) {
      js_code_content <- c(
        js_code_content,
        xfun::file_string(get_pathfile_from_res("includes/rmd_to_html", "header_code_folding.js"))
      )
      js_code_menu <- gsub(
        "[[:space:]]*/\\*Rb3m:code_folding([[:print:][:space:]]*)/Rb3m:code_folding\\*/[[:space:]]*",
        "\\1", js_code_menu
